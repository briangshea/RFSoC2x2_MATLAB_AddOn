...
#Disable the ADC RTS Ports on rfcd if they are not enabled
if { $enable_dac_rts == "false" } {
    set_property -dict [list CONFIG.DAC_RTS {false}] [get_bd_cells radio/rfdc]
}
...
if { $dac0_enable == "false" } {
    connect_bd_intf_net -boundary_type upper [get_bd_intf_pins radio/M00_AXIS_DAC_RE] [get_bd_intf_pins radio/S00_AXIS_DAC_RE]
    connect_bd_intf_net -boundary_type upper [get_bd_intf_pins radio/M00_AXIS_DAC_IM] [get_bd_intf_pins radio/S00_AXIS_DAC_IM]
}

if { $dac1_enable == "false" } {
    connect_bd_intf_net -boundary_type upper [get_bd_intf_pins radio/M10_AXIS_DAC_RE] [get_bd_intf_pins radio/S10_AXIS_DAC_RE]
    connect_bd_intf_net -boundary_type upper [get_bd_intf_pins radio/M10_AXIS_DAC_IM] [get_bd_intf_pins radio/S10_AXIS_DAC_IM]
}
